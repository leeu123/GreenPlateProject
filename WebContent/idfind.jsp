...
throw new Exception("데이터베이스에 연결할 수 없습니다.<br>");
	...
	ResultSet rs = stmt.executeQuery("select * from membership where email = '"+email+"';");
		if(rs.next()){%>
			...
			<p><span style = "font-size : 25px; font-weight : bold">아이디 찾기</span></p>
			고객님의 정보와 일치하는 아이디 입니다.<br><br>

		  <div style = "width : 800px; height : 100px; border : 1px solid">
		
			 <div style = "margin-top : 35px; margin-left : 320px">
			 <form action = "loginpage.jsp" method = post name = "id">
			 <input type = "radio" name = "finded_id" >
			 <span style = "font-size : 16px; font-weight : bold;">
			  <% out.println(rs.getString("id"));
			  session.setAttribute("finded_id", rs.getString("id") );
			  %>
			 </span>
			 </div>
			  </div>
		...
		<%}else{
			 response.sendRedirect("idfind_false.jsp");
		}
